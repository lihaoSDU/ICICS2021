...
class DDNL2Attack(Attack, LabelMixin):
    ...
    def perturb(self, x, y=None):
        ...
        s = self.clip_max - self.clip_min  # value = 1.
        ...
        for i in range(self.nb_iter):
            ...
            norm.mul_(1 - (2 * is_adv.float() - 1) * self.gamma)  # equals to: norm = norm.__mul__(alpha)

            delta.data.mul_((norm / delta.data.flatten(1).norm(p=2, dim=1)).view(-1, *data_dims))
            delta.data.add_(x)
            if self.quantize:
